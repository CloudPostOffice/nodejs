...
class Device {
  constructor(deviceId, deviceSecret, options = {}) {
    this._id = deviceId;
    this._secret = deviceSecret;
  ...
  /**
   * Send a message to another device on the same account/project.
   *
   * @param {{ to: string, msg: any }} options
   */
  async send({ to, msg }) {
    if (!to) throw new Error('send() requires a "to" field with the target device ID');
    await this._connect();
    const topic = this._topic(`devices/${to}`);
   ...
   * Returns the device so calls can be chained.
   *
   * @param {Function} callback  fn(message) — message is { from, msg, ts }
   */
  async listen(callback) {
    await this._connect();
    const topic = this._topic(`devices/${this._id}`);
  ...
  async _authenticate() {
    ...
    try {
      response = await fetch(`${this._baseUrl}/api/authenticate`, {
        ...
        body: JSON.stringify({ deviceId: this._id, deviceSecret: this._secret }),
      });
    } catch (networkError) {
  ...
  _connectMqtt() {
    return new Promise((resolve, reject) => {
      const timer = setTimeout(
        () => reject(new Error(`MQTT connection timed out for device "${this._id}"`)),
        CONNECT_TIMEOUT_MS
      );
  ...
  _reconnectMqtt() {
    return new Promise((resolve, reject) => {
      const timer = setTimeout(
        () => reject(new Error(`MQTT reconnect timed out for device "${this._id}"`)),
        CONNECT_TIMEOUT_MS
      );
