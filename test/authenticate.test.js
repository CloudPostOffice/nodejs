...
const DEVICE_ID = 'device-a8f3k';
const DEVICE_SECRET = 'mysecret1234abcd';
  ...
  describe('authenticate()', () => {
    describe('input validation', () => {
      it('throws if deviceId is missing', async () => {
        await expect(client.authenticate('', DEVICE_SECRET))
          .rejects.toThrow('deviceId and deviceSecret are required');
      });

      it('throws if deviceSecret is missing', async () => {
        await expect(client.authenticate(DEVICE_ID, ''))
          .rejects.toThrow('deviceId and deviceSecret are required');
      });
    ...
    describe('successful authentication', () => {
      const successResponse = {
        ...
        clientId: 'acc_x7k2p9:project-free:device-a8f3k',
      };

      beforeEach(() => mockFetch(200, successResponse));

      it('returns token, broker, and clientId', async () => {
        const result = await client.authenticate(DEVICE_ID, DEVICE_SECRET);
        expect(result).toEqual(successResponse);
      });

      it('sends a POST request to /api/authenticate', async () => {
        await client.authenticate(DEVICE_ID, DEVICE_SECRET);
      ...
      it('sends deviceId and deviceSecret in the JSON body', async () => {
        await client.authenticate(DEVICE_ID, DEVICE_SECRET);
        expect(global.fetch).toHaveBeenCalledWith(
          expect.any(String),
    ...
    describe('error handling', () => {
      it('throws with status 401 on invalid credentials', async () => {
        ...
        const err = await client.authenticate(DEVICE_ID, 'wrong-secret').catch(e => e);
      ...
      it('throws with status 429 on rate limit', async () => {
        ...
        const err = await client.authenticate(DEVICE_ID, DEVICE_SECRET).catch(e => e);
        expect(err.message).toBe('Too many requests');
        expect(err.status).toBe(429);
      });

      it('throws with status 400 when required fields are rejected server-side', async () => {
        mockFetch(400, { error: 'deviceId and deviceSecret are required' }, false);
        const err = await client.authenticate(DEVICE_ID, DEVICE_SECRET).catch(e => e);
        expect(err.message).toBe('deviceId and deviceSecret are required');
